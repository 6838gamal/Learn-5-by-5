...
import { Heart, CreditCard, Home, AlertTriangle, DollarSign, CheckCircle, Instagram, Linkedin, Facebook } from "lucide-react";
...
export default function DonatePage() {
  ...
  const handleDonation = () => {
    ...
    if (selectedAmount === "custom" && (!customAmount || parseFloat(customAmount) <= 0)) {
      toast({
        variant: "destructive",
        title: t('donateToastInvalidAmountTitle'),
        description: t('donateToastInvalidAmountDescription'),
      });
      return;
    }
    
    toast({
      title: t('donateToastThankYouTitle'),
      description: t('donateToastThankYouDescription', { amount: amountToDonate }),
  ...
  const donationAmounts = [
    ...
    { value: "50", label: "$50" },
    { value: "100", label: "$100" },
    { value: "custom", label: t('donateCustomAmount') },
  ];
      ...
      <Card className="w-full max-w-2xl mx-auto shadow-2xl rounded-xl overflow-hidden">
        ...
        <CardContent className="p-6 sm:p-8 space-y-8 bg-background">
          <Alert variant="default" className="bg-amber-50 border-amber-400 text-amber-700">
            ...
            <AlertTitle className="font-semibold">{t('donateNoteTitle')}</AlertTitle>
            <AlertDescription>
              {t('donateNoteDescription')}
            </AlertDescription>
          </Alert>
          ...
          <div>
            ...
            {selectedAmount === "custom" && (
              <div className="mt-6">
                <Label htmlFor="custom-amount-input" className="text-base font-medium text-muted-foreground mb-1 block">{t('donateCustomAmountLabel')}</Label>
                <Input
          ...
          <div className="space-y-3 pt-4 border-t">
            <Label className="text-xl font-semibold text-foreground mb-3 block text-center">
                ...
                {t('donatePaymentMethodsTitle')}
            </Label>
            <div className="p-4 border rounded-lg bg-muted/30">
              <p className="text-sm text-muted-foreground text-center mb-4">
                {t('donatePaymentMethodsDescription')}
              </p>
          ...
          >
            <Heart className="mr-2.5 h-6 w-6" /> {t('donateButton')}
          </Button>
